...
def safe_read_par(par_bytes):
    processed_lines = []
    content = par_bytes.decode("utf-8", errors="ignore").splitlines()
    for line in content:
        line = line.strip()
        if not line:
            continue
    ...
    return pd.DataFrame(normalized_data, columns=col_names)
...
st.title("📊 DAT & PAR to Excel Converter (Web Version)")
...
if st.button("Convert to Excel"):
    if not dat_file or not par_file:
        ...
        try:
            df_par = safe_read_par(par_file.read())
            output = io.BytesIO()
